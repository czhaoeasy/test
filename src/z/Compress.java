...
final class Compress {
	...
	public static void spec_select_action(InputStream inStream,
			OutputStream outStream, int action) throws IOException {
		byte[] inBuff = new byte[1024];
		byte[] outBuff = new byte[2048];
		int count = 0;
		count = inStream.read(inBuff);

		Input_Buffer in = new Input_Buffer(count, inBuff);
		Output_Buffer out = new Output_Buffer(outBuff);

		if (action == 0) {
			Compressor comp = new Compressor(in, out);
			comp.compress(inStream, outStream);
		}
	}
};
...
final class Compressor extends Comp_Base {
	...
	public void compress(InputStream inStream, OutputStream outStream)
		...
		do {
			...
			outStream.write(Output.getOutBuff(), 0, Output.count());

			inBuff = new byte[1024];
			outBuff = new byte[2048];
			bytesRead = inStream.read(inBuff);
			Input = new Input_Buffer(bytesRead, inBuff);
			Output = new Output_Buffer(outBuff);
